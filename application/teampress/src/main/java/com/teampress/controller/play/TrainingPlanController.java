...
public class TrainingPlanController extends BaseController {
    ...
    public ModelAndView doSubmit(@Valid @ModelAttribute("modifyTrainingPlan") TrainingPlanForm form, BindingResult result) {
        if(hasPermission(Permissions.TRAIN_CREATE)){
            ...
            TrainingPlan trainingPlan = (isNull(form.id)) ? new TrainingPlan() : trainingPlanService.findUnique(form.id);
            ...
            boolean hasPlanOnDate = trainingPlanService.existUnique(trainingPlan.getOrganization(), trainingPlan.getTeam(), trainingPlan.getTrainingDate());
            if (isNull(form.id) && hasPlanOnDate) {
                ModelAndView errorView = show();
            ...
            if (Objects.nonNull(form.getData()) && !"".equals(form.getData())) {
                ...
                TrainingPlan plan = trainingPlanService.findUnique(trainingPlan.getOrganization(), trainingPlan.getTeam(), trainingPlan.getTrainingDate());
                if (exercises.length > 0) {
                    for (TrainingPlanConnection connection : connectionService.findByTrainingPlanSorted(plan)) {
    ...
    public String del(@RequestParam String id) {
        if(hasPermission(Permissions.TRAIN_CREATE)) {
            TrainingPlan trainingPlan = trainingPlanService.findUnique(Integer.parseInt(id));
            ...
            if (!attendanceService.existByTrainingPlan(trainingPlan) && !workoutService.existByTrainingPlan(trainingPlan)) {
                ...
                for (TrainingPlanConnection connection : connectionService.findByTrainingPlanSorted(trainingPlan)) {
                    connectionService.delete(connection);
                }
                connectionService.flush();
        ...
        if(hasPermission(Permissions.TRAIN_CREATE)){
            ...
            if (!attendanceService.existByTrainingPlan(trainingPlan) && !workoutService.existByTrainingPlan(trainingPlan)) {
                ...
                try{
                    ...
                    for (TrainingPlanConnection planConnection : connectionService.findByTrainingPlanSorted(trainingPlan)) {
                        exercises.put(
